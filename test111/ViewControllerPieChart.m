...
@implementation ViewControllerPieChart
...
-(void)viewDidLoad
{
    ...
    NSLog(@"hhhhhhh");
    //  NSTimer* test=[NSTimer scheduledTimerWithTimeInterval:1 target:self selector:@selector(test) userInfo:nil repeats:YES];
}
...
-(void)viewDidAppear:(BOOL)animated
{
	...
	rotation.fromValue			 = [NSNumber numberWithFloat:M_PI * 0];
...
-(void)didRotateFromInterfaceOrientation:(UIInterfaceOrientation)fromInterfaceOrientation
{
	if ( UIInterfaceOrientationIsLandscape(fromInterfaceOrientation) ) {
		...
		pieChartView.frame	  = CGRectMake(0.0f, 0.0f, 1024.0f, 768.0f);
	}
	else {
...
-(void)constructPieChart
{
    ...
    NSString *name[]={@"hello",@"baby",@"fuck"};
	// Add some initial data
    // NSDictionary *bar=[NSDictionary dictionaryWithObjectsAndKeys:[NSNumber numberWithDouble:<#(double)#>], nil]
	NSMutableArray *contentArray = [NSMutableArray arrayWithObjects:[NSNumber numberWithDouble:40.0], [NSNumber numberWithDouble:60.0], [NSNumber numberWithDouble:NAN], [NSNumber numberWithDouble:100.0], nil];
	self.dataForChart = contentArray;
    //NSMutableArray *contentArray1 = [NSMutableArray arrayWithObjects:name[0],nil];
    //self.dataForPlot=contentArray1;
}
...
-(CPTLayer *)dataLabelForPlot:(CPTPlot *)plot recordIndex:(NSUInteger)index
{
	...
	if ( !whiteText ) {
        ...
        whiteText.fontSize=20.0;
        whiteText.fontName=@"Verdana-Bold";
	}
    
	CPTTextLayer *newLayer = nil;
    NSArray *temp=[NSArray arrayWithObjects:[NSString stringWithFormat: @"conditioner(20percent)"],[NSString stringWithFormat: @"light(30percent)"],[NSString stringWithFormat: @"computer"],[NSString stringWithFormat: @"TV(50percent)"],nil];
    
    NSString *name =@"hahaFuck";
    //   NSLog(@"fdfgdfgdfgdgf");
	...
	if ( [plot isKindOfClass:[CPTPieChart class]] ) {
		switch ( index ) {
                NSLog([temp objectAtIndex:index]);
			case 0:
                newLayer = [[CPTTextLayer alloc] initWithText:[NSString stringWithFormat: [temp objectAtIndex:index]] style:whiteText];
